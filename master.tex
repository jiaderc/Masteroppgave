...
\\
\\
A confusion matrix gives an insight of which classes were easier to predict than others. The confusion matrix consists of a square matrix with one row for predicted class, and a column for actual class (or vice-versa). Consider the confusion matrix of the 3-class classification in table \ref{tab:cmatrix}. The model got correct classification of all instances that belongs to class $C_1$. Meanwhile the model misclassified 5 instance of $C_2$ as $C_3$, and not able  getting the class $C_3$ correct. 
