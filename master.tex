...
\begin{enumerate}
	...
	\item Selecting two best performer to find best hyperparameter and test the performance
	\item Using the best performed classifier to evaluate the performance when the robot walks through two different terrains.
	\item Using the training sample to predict other sensor. 
\end{enumerate}


\chapter{Experiments and results}                     %% ... or ??
The chapter presents results of all models in the previous chapter. The result of classification is in will first be presented. Further improvement for  
...
\subsection{Analysis}
The result shown that all classifier has accuracy of at least 70\%. The Decision tree and Neural Network has the highest accuracy with 96\% from scaled feature set one. 
\\
\\
...
\\
\\
Beside of looking at the result, it is also interesting to look how data from sensor differ from the other.
...
\subsection{Result of added feature selection}
