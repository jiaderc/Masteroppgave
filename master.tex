...
\section{Optical force sensor}
The sensor used in this thesis is OptoForce (3D force sensor) \cite{Optoforce} is similar to Tar and Cserey \cite{6027100}. A construction of the sensor is shown on the figure \ref{fig:OptoforceBuild}. 
\begin{figure}[h]
	\centering
...
The robot operating on ROS which is compatible with C++, Python and Lisp. Segmentation of data, described later in section \ref{subseq:segmentation}, is implemented in C++, due to fast computation. However, C++ does not provide many learning libraries, thus the learning approach is implemented in Python. 
...
\begin{enumerate}
	...
	\item The learning process will be integrating two different feature selection methods to achieve a better performance. 
	\item Some of the top performer classifier along with the feature set will be further tested on new data samples.
	\item Selecting two best performer and use grid search to find best parameters for each classifier and test on the previous data samples.
	\item The best performed classifier will be to evaluate the performance when the robot walks through two different terrains.
...
\\
\\
In this experiment 50 new samples for each terrain is collected. In the cross validation it will use the old dataset as training set, while the new collected samples will be used as test file. The cross validation is still LOOCV. The training and test set will be randomly shuffled before the cross validation. Thus, to achieve more precise measurement, each test will be running five times. It is claimed in that 
...
Previously, the evaluation of each classifiers was based on default hyperparameters. However, tuning parameters might increase the performance of the classifier. To find optimal parameters will by using a grid search. Parameter values tested can be seen in table \ref{tab:para}. The best parameters will be further used to test the performance. Note that the output of the Neural Network only has been a string which is the name of the terrain. However, it will be interesting to see if there any effect if there are more output. Thus, for the further experiment it will also test with binary with length of 2 or 4.
\\
\\ 
Every classifier has parameters that can be optimized. Using default values is more unlikely that this is suitable for terrain classification. However, choosing a correct set of hyper parameters might increase its performance. A drawback of searching parameters is time-consuming if there are many parameters to be tested. 
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lll@{}}
		...
		\multirow{2}{*}{Neural Network} & Hidden layer & 0,...,100 \\
		& Alpha & 0.0001,0.0002,0.0010 \\ \midrule
		\multirow{3}{*}{SVM} & Kernel & linear, rbf, sigmoid \\
		& Gamma & 0.001,0.002,....,0.1 \\
		& C & 0.1,0.2,...,1,2,...,1000 \\ \bottomrule
	\end{tabular}
	\caption{Table showing the hyperparameters used in the exhaustive grid search
		for Neural Network and SVM.}
	\label{tab:para}
\end{table}
\FloatBarrier
...
In following paragraphs present the results along with analysis of each experiments from... TODO
...
The achieved parameters from grid search did not show a big effect on the performance. However, it is stated in that  . For neural network there are many other to be tested, output blablabla
...
\chapter{Algorithms}\label{ap:featuresel}

\section{Removing features with low variance} \label{ap:variance}
This feature selection algorithm provided from Python removes all features whose variance does not meet some threshold. The default value will remove all features with the same values, which is most unlikely will occur in this experiment. Thus, in experiments the method will be removing feature when the variance is less than 0.2. 

\section{Recursive feature elimination} \label{ap:rfe}
This feature selection algorithm will be using a supervised classifier to rank each features and remove features with low rank. The process is let a estimator to train of the features and weights thems. Features with smallest weights are pruned, and start to estimate with the currently features. This will be repeated until the desired number of features is reached.
