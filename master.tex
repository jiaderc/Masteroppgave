...
\FloatBarrier
\subsection{Results}
An overview of the performance is shown as box plot in figure \ref{fig:boxexp}. Again, the top performing is the SVM, and second best is the Neural network with feature set five, and will be used in further experiments. The other classifier has decreased their accuracy in their performance.
...
Hyperparameters that will be search for SVM is shown in table \ref{tab:hypsvm}. The kernel can be seen as a similarity function. As each of kernel has a different way to compute their similarities. Specifications of each kernels can be seen in appendix \ref{ap:kernel} . When one not wanting a perfect separation between all data, one can allow some misclassification by tuning the parameter C. C tells how much one want to avoid misclassifying each training examples. Large C gives a more perfect separation of the data, but might not generalize data so good, while low C value allows more misclassification, and might contains more error. The gamma parameter only influence RBF due to the similarity computation.
...
\subsection{Results}
In table \ref{tab:resPara} shows the result after tuned parameter. The best result is marked with bold text and will be used in further experiments
...
\subsection{Analysis}
As seen in table \ref{tab:resPara}, it has not shown significant effect of the performance. Mostly is slightly decrease, but neural network with two outputs has minor increased. Again, the top performing is the SVM with feature set 5. 
\\
\\
Regarding binary representation of terrains, did not show any significant improvement. A decreased of accuracy when the binary pattern is on four. A minor increasing of accuracy when the output consists of two binary outputs.   
...
\begin{table}[h]
	...
	\begin{tabular}{ll}
		...
		&  \\
		Linear & $(1 + \vec{x} \cdot \vec{y})^{d}$  \\ \hline
	\end{tabular}
