...
Humans naturally adapt their walking style on different terrains to obtain a stable locomotion. Adaptation of walking style comes from past experience. For instance, one would not run on icy sidewalk in order to prevent from slipping or falling. While on more rough surface, one may freely decide the speed to move without worrying about losing control of the motion. To achieve this ability on a robot, the robot must be able to detect and distinguish different terrains.
\\
\\
Terrain classification is the process of identifying different types of terrain by measure features such as texture, slope, roughness, hardness, and friction. It is a popular research field where countless of studies can be found in the literature  \cite{Giguere06environmentidentification,Giguere2009,6386243,6569179,4399500,littleDog}. Some of the importance of terrain classification is shown in \cite{7487541}, where different controllers were suited for different terrains by letting a quadruped robot hop on soft and hard terrain. Another study investigated the effect of performance with different gait parameters on different terrains \cite{6569179}. The results indicated there is a trade-off between the energy consumption and physical speed of the robot by controlling the velocity of the leg motors on different types of terrains.
\\
\\ 
%Sucessful perception of the terrain is a key ability that impacts the gait change 
Robot's perception of different surfaces plays an important rule for successful terrain classification. Researchers often obtain features from terrain from a distance such as camera \cite{littleDog} or laser scanners \cite{4651026}. Other studies measure properties through robot's interaction on different surfaces such as leg joints \cite{26b23e912c654fe4b7478fd910130195}, and accelerometers \cite{DBLP:conf/emcr/WeissFSZ07}, 
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.22\textwidth}
		...
		\label{fig:LAURON}
	\end{subfigure}
	\caption[Overview over different types of legged robot]{Overview over different types of legged robot.}\label{fig:robots}
\end{figure}
...
Based on the these information listed above from the tactile sensor, it is possible to use the robot for manipulation, exploration or response. These activities are shown in figure \ref{fig:tactiles}. For manipulation it means that it is possible to control, for instance, the grip force on an object. Exploration has the possibility of identifying objects by assimilating information about properties such as hardness, friction, and roughness from materials and surfaces. Response refers to detection of, and reaction to, contact from external agents, and sensing if it is a gentle touch or strong impact.
	...
	\begin{subfigure}[b]{0.32\textwidth}
		...
		\label{fig:tact3}
	\end{subfigure}\hfill
	\caption[Applications for the tactile sensor]{Three types of application that can be used of the tactile sensor which is to manipulate (a) exploration (b) response (c) \cite{Cutkosky2008}.}
	\label{fig:tactiles}
\end{figure}
...
Supervised learning algorithms predict new data based on a labeled dataset. That is, the system in the learning process knows the correct answers of each dataset, and is the basis for the prediction. The learning process usually stops when the algorithm converges towards an acceptable level of performance.
	
\paragraph{Unsupervised learning}
Unsupervised learning algorithms make predictions from data points without labels. The system has to organize the data on its own and is the basis for predictions. A common method to organize the data is by clustering.
...
\begin{figure}[h]
	...
	\caption[Model of a multi-layer perceptron]{Model of multi-layer perceptron with one hidden layer \cite{MLP}.}
	\label{fig:MLP}
\end{figure}
...
\begin{figure}[h]
	...
	\caption[An example of SVM classificiaton]{Model of optimal margin and hyperplane created by SVM on a two-class classification problem  \cite{Cortes1995}.}
	\label{fig:SVM}
\end{figure}
\FloatBarrier
...
\begin{figure}[h]
		...
		\caption[An example of KNN classification problem]{An example of a KNN classification \cite{KnnClassification}. The green circle is to be classified to either blue square or red triangle. The solid circle is when k = 3, while the dashed circle is when k = 5.}
		\label{fig:KNN}
\end{figure}
...
\begin{figure}[h]
		...
		\caption[Model of a decision tree]{An example of a simple decision tree \cite{Quinlan1986}. This decision tree classify a day to be P or N based on outlook, humidity and windy.}
...
Overfitting and underfitting the data is an issue in machine learning which causes poor performance of classification. Overfitting occurs if the classifier has adapted to the noise within a data set. Meanwhile, underfitting occurs if there is not enough training data and the classifier will not be able to generalize a new data set. An example for each case is shown in figure \ref{fig:fitting}. The cross-validation estimates how accurately the classifier model will perform in practice, which might prevent overfitting or underfitting the data.
	...
	\begin{figure}[h]
		...
		\begin{subfigure}{\linewidth}
			...
			\label{fig:optimal}
		\end{subfigure}
		\caption[Illustration of when a model is over-, underfitted and optimal]{Illustration of when a model is overfitted \ref{fig:over}, underfitted \ref{fig:under} and optimal \ref{fig:optimal}}
		\label{fig:fitting}	
	\end{figure}
...
The process of K-fold is to partition the data into k subsets, where one subset is used for testing and the other is used for training. When the trained model has assessed the test set, a new subset is selected as the test set. This process is repeated k times, that is when all subsets have been used as a test set. Setting k to the length of feature vectors is also known as leave-one-out cross-validation (LOOCV). LOOCV only uses one feature vector as a test set, while the rest as a training set, shown in figure \ref{fig:kfold}. Estimations based on LOOCV tend to be almost unbiased, but unreliable due to high variance. However, the advantage is that it uses as many training samples as possible and is widely used especially when there are only small amounts of data available.
	...
	\begin{figure}[h]
		...
		\caption[K-fold cross validation]{An instance of k-fold cross validation. In this case is the LOOCV, since the k is set to be 8 which is the length of the feature vectors.}
		\label{fig:kfold}
	\end{figure}
...
It is interesting to see which class is easier to predict than others. This can be seen by using the confusion matrix. The confusion matrix consists of a square matrix with one row for predicted class, and a column for actual class (or vice-versa). An example of the confusion matrix is shown in table \ref{tab:cmatrix}.
	...
	\begin{table}[h]
		...
		\end{tabular}
		\caption[Confusion matrix of 3-class classification.]{Confusion matrix of 3-class classification.}
		\label{tab:cmatrix}
	\end{table}
...
This chapter attempt to present the environment setup, and explain the various choices which have been made to create machine learning models.
...
\begin{figure}[h]
	...
	{\small Four different terrain used in all experiments which are floor \ref{fig:mean14}, carpet \ref{fig:mean24}, soft mat \ref{fig:mean34}, and hard mat \ref{fig:mean44} } 
	\label{fig:terrains}
\end{figure}
\FloatBarrier
...
\begin{figure}[h]
	...
	\caption[Process of creating a training file]{Process of creating a training file.} \label{fig:createsamples}
\end{figure}
...
\begin{figure}[h]
	...
	\caption[Process to create and evaluate a certain classifier]{Process to create and evaluate a certain classifier.} \label{fig:learningProcess}
\end{figure}
...
\\
The classifier will take the training set as input and train. The parameters of each classifier is default values from Python. 
...
\begin{table}[h]
	...
	\caption[Results from five different feature sets and classifier withtout feature selection method]{Results after following the approach shown in figure \ref{fig:learningProcess}. Colored green has an accuracy over 90\%, yellow is between 80\%-89\%, while red has accuracy under 79\%}
	\label{exp1}
\end{table}
\FloatBarrier
...
\begin{figure}[h] \ContinuedFloat
	...
	\caption[Example of mean the each terrain for each directions in the time domain]{A mean of 10 steps on each terrain in different directions in the time domain, where mean in the x-direction \ref{fig:meanx}, mean in y-direction \ref{fig:meany}, and mean in z-direction \ref{fig:meanz}  is computed.}
	\label{fig:meanxyz}
\end{figure}
...
\begin{figure}[h] \ContinuedFloat	
	...
	\caption[Examples of mean the each terrain for each directions in the frequency domain]{A mean of each terrain different direction in frequency domain, where mean in the x-direction \ref{fig:fftx}, mean in y-direction \ref{fig:ffty}, and mean in z-direction \ref{fig:fftz} is computed.}
	\label{fig:fftxyz}
\end{figure}
\FloatBarrier
...
\begin{table}[h]
	...
	\end{tabular}
	\caption[Results of length of feature vector after feature selection method]{New length of each features set after applied feature selection method.}
	\label{tab:exp2_1}
\end{table}
\FloatBarrier
...
\begin{table}[h]
	...
	\caption[Results from five different feature sets and classifier with feature selection method]{Results from five different feature sets and classifier when applied feature selection method}
	\label{tab:exp2}
\end{table}
\FloatBarrier
...
\begin{table}[h]
	...
	\end{tabular}
	\caption[Overview of top performing approaches to be tested on unseen data samples]{Overview of top performing classifiers with its feature set which will be used to test on unseen data.}
	\label{tab:exp4}
\end{table}
\FloatBarrier
\newpage
...
\begin{figure}[h]
	...
	\caption[Boxplot of five top performing classifier on unseen data]{Boxplot of five top performing classifiers.}
	\label{fig:boxexp}
\end{figure}
\FloatBarrier
\subsection{Analysis}
...
\begin{table}[h]\ContinuedFloat
	\begin{subtable}[h]{1\textwidth} 
		...
		\end{tabular}
		\caption{Accuracy and confusion matrix for SVM with wrapper using feature set three after five runs. Regarding confusion matrix, the row is the actual terrains and the columns is the predicted terrains.}
		\label{subt:svmexp}
	\end{subtable}
	\caption[Results of SVM - RFE - feature set three]{Results of SVM with RFE on feature set three after five runs, with precision \ref{pressvm}, recall \ref{recallsvm}, f-score \ref{fscoresvm}, and accuracy and confusion matrix \ref{subt:svmexp}.}
	\label{svmexp}
\end{table}
\FloatBarrier
\newpage
...
\begin{table}[h]\ContinuedFloat
	\begin{subtable}[h]{1\textwidth} 
		...
		\end{tabular}
		\caption{Accuracy and confusion matrix for neural network and feature set five after five runs. Regarding confusion matrix, the row is the actual terrains and the columns is the predicted terrains.}
		\label{nnset5}
	\end{subtable}
\caption[Results of Neural network - feature set five]{Results of neural network using feature set five after five runs, with precision \ref{tabl:nnset5pre}, recall \ref{tab:nnset53}, f-score \ref{tab:nnset5fscore}, and accuracy and confusion matrix \ref{nnset5}.}
\label{tab:binary}
\end{table}
\FloatBarrier
\newpage
...
\begin{table}[h]\ContinuedFloat
	\begin{subtable}[h]{1\textwidth} 
		...
		\end{tabular}
		\caption{Accuracy and confusion matrix for decision tree with RFLV using feature set one after five runs. Regarding confusion matrix, the row is the actual terrains and the columns is the predicted terrains.}
		\label{dt4}
		\end{subtable}
	\caption[Results of SVM - RFE - Feature set three]{Results of decision tree with RFLV using feature set one after five runs, with precision \ref{dt1Precision}, recall \ref{dt1recall}, f-score \ref{dtfscore}, and accuracy and confusion matrix \ref{dt1e}.}
\label{tab:binary}
\end{table}
\FloatBarrier
\newpage
...
\begin{table}[h]\ContinuedFloat
	...
	\end{tabular}
	\caption{Accuracy and confusion matrix for decision tree using feature set one after five runs. Regarding confusion matrix, the row is the actual terrains and the columns is the predicted terrains.}
	\label{dt1e}
	\end{subtable}
	\caption[Results of Decision Tree - Filter - Feature set one]{Results of decision tree with RFLV on feature set three with precision \ref{dt1Precision}, recall \ref{dt1recall}, f-score \ref{dtfscore}, and accuracy and confusion matrix \ref{dt1e}.}
\label{tab:dta1ll}
\end{table}
\FloatBarrier
\newpage
...
\begin{table}[h]\ContinuedFloat
	\begin{subtable}[h]{1\textwidth} 
		...
		\label{nn2}
	\end{subtable}
	\caption[Results of Neural Network - Feature set one]{Results of neural network using feature set three after five runs, with precision \ref{nnPrecision}, recall \ref{nnrecall}, f-score \ref{nnfscore}, and accuracy and confusion matrix \ref{nn2}.}
	\label{tab:binary}
\end{table}
...
\begin{table}[h]
	...
	\end{tabular}
		\caption{Two binary digits representation of each terrain.}
		\label{tab:twobin}
	\end{subtable}
	\hfill
	\begin{subtable}[h]{0.5\textwidth}
...
\end{tabular}
		\caption{Four binary digits representation of each terrain.}
		\label{tab:fourbin}
	\end{subtable}
...
\begin{table}[h]
	...
	\caption[Results of classifier after tuned parameters]{Results after tuned parameters of Neural Network and SVM.}
	\label{tab:resPara}
\end{table}
\FloatBarrier
...
A more realistic practical scenario is let the robot traverse through different terrains. Thus, the earlier approach has to be modified due to this experiment. In this experiment, the robot will be walking on two different terrains. There are in total five different setups.
...
The data stream when the robot traverse from floor to carpet is shown in figure \ref{fig:gulvet4teppegraf}, where the vertical line is the transition between the terrains, and the table \ref{tab:Gulvet4teppe} shows probabilities for each terrain on each step. The overall accuracy of predicting correctly is 88.9\%. However, how certain the classifier is to predict correctly is under 78\%. The fifth step is the transition and as shown, there is some confusion between floor and carpet. It is also worth noticing that at the fourth step the carpet has almost the same probability as the floor with 56.4\% and 42.5\% respectively. The soft mat and hard mat have small probabilities in every step and is more unlikely to be predicted.
	...
	\begin{table}[h]
		...
		\caption[Results of transistion from floor to carpet]{Estimated probability of each terrain per step from floor to carpet. Marked green represent correct prediction and correct terrain, red represent wrong prediction and yellow is the correct prediction if it got wrong.}
...
\begin{figure}[h]
	...
	\caption[Data stream of the robot traversing from hard mat to floor]{Data stream of the robot traversing from hard mat to floor. The thick gray horizontal line is the boundary between the two terrains.}
	\label{fig:mb3Gulvet}
\end{figure}
...
\begin{table}[h]
	...
	\caption[Results of transistion from hard mat to floor]{Estimated probability of each terrain per step walking from hard mat to floor. Marked green represent correct prediction and correct terrain, red represent wrong prediction and yellow is the correct prediction if it got wrong.}
	\label{tab:mb3Gulvet}
\end{table}
\FloatBarrier
\clearpage
...
\subsubsection{Hard mat to soft mat} \label{sec:hmssm}
 The data stream when the robot traverses from hard mat to soft mat is shown in figure \ref{fig:hardmatSoftMat} and the table \ref{hardmatSoftMat} shows probabilities for each terrain on each step. The overall accuracy of predicting correctly is 100\%, while the average of choosing the correct terrain is 72.7\%. The forces in the y- and z-directions are much higher when walking on hard mat than soft mat. There is some uncertainty on the first, third and sixth step, where the probability of predicting correctly is under 54\%.
	...
	\begin{figure}[h]
		...
		\caption[Data stream of the robot traversing from hard mat to soft mat.]{Data stream of the robot traversing from hard mat to soft mat. The thick gray horizontal line is the boundary between the two terrains.}
	...
	\begin{table}[h]
		...
		\caption[Results of transistion from hard mat to floor]{Estimated probability of each terrain per step walking from hard mat to soft mat. Marked green represent correct prediction and correct terrain, red represent wrong prediction and yellow is the correct prediction if it got wrong.}
...
The data stream when the robot traverses from soft mat to hard mat is shown in figure \ref{fig:MM_4_Resten_BGraf}, and the table \ref{MM4MB} shows probabilities for each terrain on each step. The overall accuracy of predicting correctly is 100\%, while the average of choosing the correct terrain is 79\%. Again, the forces in the y-, and z-directions are much higher when walking on hard mat than soft mat. There is some uncertainty on the first and fourth steps, where the probability of predicting correctly is under 53.5\%.
	...
	\begin{figure}[h]
		...
		\caption[Data stream from soft mat to hard mat]{Figure showing data stream from sensor when the robot walked from soft mat to hard mat. The thick gray horizontal line is the boundary between the two terrains.}
	...
	\begin{table}[h]
		...
		\caption[Results of transistion from soft mat to hard mat]{Estimated probability of each terrain per step walking from soft mat to hard mat. Marked green represent correct prediction and correct terrain, red represent wrong prediction and yellow is the correct prediction if it got wrong.}
		\label{MM4MB}
	\end{table}
	\FloatBarrier
	\clearpage
...
The data stream when the robot traverses from soft mat to carpet is shown in figure \ref{fig:softcarpet}, and the table \ref{tab:softcarpet} shows probabilities for each terrain on each step. The overall accuracy of predicting correctly is 87.5\%. Again, the forces in the y- and z-directions are much higher when walking on carpet than soft mat. There is some uncertainty on the first and fourth steps, where the probability of predicting correctly is under 53.5\%. The fifth step is when the robot is on the new terrain, and has a probability of predicting it correct of 66.7\%. Regarding step six, the classifier makes a wrong prediction between carpet and hard mat with a probability of 93.3\%.
	...
	\begin{figure}[h]
		...
		\caption[Data stream of the robot traversing from soft mat to carpet.]{Data stream of the robot traversing from soft mat to carpet. The thick gray horizontal line is the boundary between the two terrains.}
	...
	\begin{table}[h]
		...
		\caption[Results of transistion from soft mat to carpet]{Estimated probability of each terrain per step walking from soft mat to carpet. Marked green represent correct prediction and correct terrain, red represent wrong prediction and yellow is the correct prediction if it got wrong.}
...
\begin{figure} [h]
	...
	\begin{subfigure}[b]{\textwidth}
		...
		\end{subfigure}
		\caption[Comparision between data stream sensor mounted on left and right front foot of the robot]{Comparision between data stream sensor mounted on left and right front foot, on floor \ref{fig:s1gulv2}, carpet \ref{fig:s1teppe}, hard mat \ref{fig:s1hardmatte}, and soft mat \ref{fig:s1mykmatte}.}
		\label{fig:s1all}
	\end{figure}	
	\FloatBarrier
...
To give an overview of the thesis performance, a comparison with earlier work is shown in table \ref{table:compareEarly}. Those approaches were all performed with local sensing and tested with unseen data. The thesis approach has among the top performers and obtains similar result as in \cite{5752869,4059113,5602459,26b23e912c654fe4b7478fd910130195} with an accuracy of at least 90\%. However, the experiments in \cite{5752869,4059113} were performed by a wheeled robot, which is more easily achieves maintained stability and less noisy data when preprocessing data from a sensor than a legged robot. The thesis approach also outperforms work which used more sensors \cite{6386243}. Even the work in \cite{26b23e912c654fe4b7478fd910130195} only used one type of sensor, demonstrates a result of 97\%.  However, the authors were incorporating all six servos from each leg which produces more data to be preprocessed. It is also worth mentioning that the work achieved higher accuracy when rocks and mulch were considered as one terrain. Having more sensors or incorporating all four sensors in this work might also increase the accuracy. The most similar comparison is with work reported in \cite{6784609}, where a similar robot platform and tactile sensor is used during the experiments. However, the work does not report the type of tactile sensor used, which makes it difficult to compare the sensor type itself. The experiments were tested on blue foil, styrofoam, linoleum, cardboard, and rubber, which have significant differences in their properties and achieved an accuracy of 84.69\%. As the robot in the thesis is able to distinguish between floor, carpet, and hard mat, it should also be able to predict all of those too.
...
\\
\\
Many of the classifiers had a high accuracy during the cross validation. However, when selecting five well performing classifiers to see the performance on unseen data, the accuracy dropped as anticipated. The neural network and decision tree, which had an accuracy of over 90\%, dropped to 64\% and 78\% respectively. The best performing classifier was SVM with an accuracy of 94.8\%, but had difficulty of predicting the hard mat.
\\
\\
...
\\
\\
The last experiment demonstrated the possibility of using training data from a sensor mounted on the front left foot to predict front right foot samples. The data provided might differ from the other, but did have some of the same characteristics. However, to achieve a higher accuracy it is better having training samples for each sensor. 
\\
\\
Compared the results obtained from cross validation with earlier work, the sensor has shown feasible results. Note that in this work, only a sensor is used to discriminate terrains, while much of the other past work have either used wheeled robots that provide more stable locomotion, or sensor fusion to achieve a good result. As the thesis sensor is able to distinguish terrains such as floor, carpet, and hard mat, which have similar properties, it is likely be able to predict outdoor terrain such as rocks and grass as well.
	
\section{Future work}
The classifier is based on the features due from data from optical force sensor. However, features of the terrain itself such as hardness and friction are not analyzed directly, which can be addressed in future work. 
\\
\\
This thesis has only experimented with flat terrains. It will be interesting to include rougher surfaces or other types and investigate the performance with the currently model. 
\\
\\
