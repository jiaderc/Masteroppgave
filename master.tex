...
Identifying different types of terrains is an important ability for every legged robot to achieve a stable locomotion. A variety of sensors have been applied in different robots in order to discriminating the terrains accurately. The tactile sensor is widely used due to the benefits of measuring properties from terrain by physical contact between the sensor and the surface. However, less has only utilized the tactile sensor on a quadruped robot in studies, and little attention was paid to the type of sensor. It exists a variety types of tactile sensors with their benefits and drawbacks. The optical sensor has a high sensitivity, small size, light weight and low detection time, which are adequate properties to distinguish between different surfaces. This thesis investigates the possibility of identifying different terrains using 3D optical force sensors and machine learning. The measurements were retrieved from a quadruped robot developed at the University of Oslo (UiO) on four different terrains. The proposed approach has the possibility of using in real time applications, and a custom segmentation method was presented for extracting desired sensor data. The segmented sensor data was the basis for creating five different feature sets and tested on five different classifiers: support vector machine, k-nearest neighbors, artificial neural network, naive Bayes, and decision tree. The experimental results demonstrated among the top performed approaches compared to earlier work with an accuracy of 94.8\% with the support vector machines.   
...
\\
\\
The importance of terrain classification is be able to change gait on different terrain. Thus, the approach should be enhanced to be able to traverse between different terrains with different gaits.
