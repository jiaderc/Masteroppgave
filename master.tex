...
Based on the information described in the list above, the tactile sensor can be used for manipulation, exploration or response. These applications are shown in figure \ref{fig:tactiles}. Using tactile sensor for manipulation is to control, for instance, the grip force on an object. Exploration reflects the possibility of identifying objects by assimilating information about properties such as hardness, friction, and roughness from materials and surfaces. Response refers to the detection of, and reaction to, contact from external agents, and sensing if it is a gentle touch or strong impact.
	...
	\begin{subfigure}[b]{0.32\textwidth}
		...
		\label{fig:tact3}
	\end{subfigure}\hfill
	\caption[Applications of the tactile sensor]{Three types of application that can be used of the tactile sensor: manipulation (a), exploration (b), response (c) \cite{Cutkosky2008}.}
	\label{fig:tactiles}
\end{figure}
...
\begin{table}[h]
	...
	\caption[Results from five different feature sets and classifiers without the feature selection method]{Results after following the approach shown in figure \ref{fig:learningProcess}. Green has an accuracy of 90\%, yellow is 80\%-89\%, while red 79\% and under.}
	\label{exp1}
\end{table}
\FloatBarrier
...
\begin{table}[h]
	...
	\caption[Results from five different feature sets and classifiers with the feature selection method]{Results from five different feature sets and classifiers when applying the feature selection method. Green is increased accuracy, yellow is same accuracy, and red is decreased accuracy.}
	\label{tab:exp2}
\end{table}
\FloatBarrier
...
\begin{table}[h]\ContinuedFloat
	\begin{subtable}[h]{1\textwidth} 
		...
		\end{tabular}
		\caption{Accuracy and confusion matrix for neural network on feature set five after five trials. Regarding the confusion matrix, the rows show the actual terrains and the columns show the predicted terrains.}
		\label{nnset5}
	\end{subtable}
...
\begin{table}[h]\ContinuedFloat
	\begin{subtable}[h]{1\textwidth} 
		...
		\label{dt1ee}
		\end{subtable}
	\caption[Results of decision tree on feature set one with RFLV]{Results of decision tree on feature set one with RFLV after five trials, with precision \ref{dt1Precision}, recall \ref{dt1recall}, f-score \ref{dtfscore}, and accuracy and confusion matrix \ref{dt1ee}.}
\label{dt44}
\end{table}
\FloatBarrier
\newpage
...
\begin{table}[h]\ContinuedFloat
	...
	\label{dt1e}
	\end{subtable}
	\caption[Results of decision tree on feature set one]{Results of decision tree on feature set one after five trials, with precision \ref{dt1Precision}, recall \ref{dt1recall}, f-score \ref{dtfscore}, and accuracy and confusion matrix \ref{dt1e}.}
\label{dt1e222}
\end{table}
\FloatBarrier
\newpage
...
\begin{table}[h]\ContinuedFloat
	\begin{subtable}[h]{1\textwidth} 
		...
		\label{nn2}
	\end{subtable}
	\caption[Results of neural network on feature set one]{Results of neural network on feature set one after trials runs, with precision \ref{nnPrecision}, recall \ref{nnrecall}, f-score \ref{nnfscore}, and accuracy and confusion matrix \ref{nn2}.}
	\label{tab:nn222}
\end{table}
