...
There are many types of kernel function as shown in \ref{tab:kern}, where each of them is more appropriate than another depending on how complex the problem is. For instance, the dot product is sufficient if a problem is linearly separable in the original space.
...
The first process is to gather data for each terrain which will be stored into files. Those files will further be collected into a single file and used as the training and test samples in the next process. The process of creating file is shown in figure \ref{fig:createsamples}.
