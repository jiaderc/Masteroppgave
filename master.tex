...
\\
\\
Walas \cite{walastactile} attached a 6-axis tactile sensor on a hexapod and the classification was only based on the sensor. Five different terrains were used: soft ground, artificial grass, gravel, pebbles, and sand. The experiments consists of robot predicts a terrain while it walks straight on the terrain. There were total 10 trials for each terrain where it consisted of six steps. The author based the classification on only one of the signal from the tactile sensor. The results showed that the information from force in x,y-direction and torque in x, y, and z-direction did not perceive informative properties from the terrain, where the most accuracy were under 60\%. However, by using data from force and torque in z-direction the author achieved an accuracy of 76.67\%. 
\\
\\
Wu et al. \cite{7397881} designed a capacitive tactile sensors mounted on a small two-legged robot. Six different terrains were used, however, the terrains were further grouped into four groups based on their friction and stiffness properties. The four classes consists of a high friction hard surface class which was a thin silicone-lined acrylic surface and a laminate tile;  a low friction hard surface class which was Teflon; a deformable surface class, which was carpet and synthetic grass; and a granular class, which was chia seeds. The two-legged robot had support from a gimbal apparatus which made the robot run in circular track with interchangeable surfaces. The results gave an accuracy of 94.36\%. However, when only used stride frequency and motor torque they got 85.53\%, while stride frequency and tactile gave accuracy of 88.31\%
\\
\\ 
...
\begin{figure}[h]
	...
	\caption[Force directions of the OptoForce sensor]{Illustration where the force from the x,y and z-directions points from the 3D OptoForce \cite{OptoforceSheet}.}
...
\begin{figure} [h]
	...
	\end{subfigure}
	\caption[Example of data stream for each terrain]{Figure showing an example data stream for each terrain: floor \ref{fig:gulvgraf}, carpet \ref{fig:teppegraf}, hard mat \ref{fig:mykmattegraf}, and soft mat \ref{fig:mattebgraf}.}
	\label{fig:gtmgraf}
\end{figure}
\FloatBarrier
